...
class SeedArtifactReuseMeasurementTest {
    @get:Rule
    val composeRule = createComposeRule()

    @Test
    fun seedSubmission_measuresUntilLiveFivePaperBriefingIsVisible() {
        val arguments = InstrumentationRegistry.getArguments()
        val input =
            MeasurementInput(
                pairId = arguments.getString(PAIR_ARGUMENT).orEmpty(),
                seedArxivId = arguments.getString(SEED_ARGUMENT).orEmpty(),
                condition = arguments.getString(CONDITION_ARGUMENT).orEmpty(),
                baseUrl = arguments.getString(BASE_URL_ARGUMENT).orEmpty(),
                token = arguments.getString(TOKEN_ARGUMENT).orEmpty(),
            )
        SeedArtifactReuseMeasurementFiles.reset()
        var stage = "arguments"
        ...
        try {
            ...
            SeedArtifactReuseMeasurementFiles.write(
                input = input,
    ...
    internal data class MeasurementInput(
        ...
        fun validate() {
            require(PAIR_PATTERN.matches(pairId)) { "The pair argument is invalid." }
            require(seedArxivId in FIXED_SEEDS) { "The seed is not in the fixed evaluation set." }
            require(condition in CONDITIONS) { "The condition must be first or reuse." }
    ...
    private companion object {
        const val PAIR_ARGUMENT = "seedReusePair"
        const val SEED_ARGUMENT = "seedReuseSeed"
        const val CONDITION_ARGUMENT = "seedReuseCondition"
        const val BASE_URL_ARGUMENT = "seedReuseBaseUrl"
        const val TOKEN_ARGUMENT = "seedReuseToken"
        const val EXPECTED_PAPER_COUNT = 5
        const val UI_TIMEOUT_MILLIS = 30_000L
        const val LIVE_TIMEOUT_MILLIS = 15 * 60 * 1_000L
        const val NANOS_PER_MILLISECOND = 1_000_000.0
        val PAIR_PATTERN = Regex("pair_0[1-3]")
        val FIXED_SEEDS = setOf("1706.03762", "2010.11929", "2106.09685")
        val CONDITIONS = setOf("first", "reuse")
    }
}
