...
internal object SeedFirstInitializationMeasurementFiles {
    ...
    private val outputDirectory =
        File(
            ...
            "seed-first-initialization-evaluation",
        )
    ...
    @Synchronized
    fun write(
        input: SeedFirstInitializationMeasurementTest.MeasurementInput,
        ...
        check(resultFile.readLines().size == 1) {
            "A seed trial must retain exactly one result row."
        }
        val values =
            listOf(
    ...
    private const val HEADER =
        "schema_version,trial_id,seed_arxiv_id,duration_ms,success,outcome," +
