...
MNEME preserves continuity across changing papers, generated assistance, and evolving research interests by maintaining explicit identities for papers, revisions, evidence, behavioral events, and mobile presentation states.
The evaluations examine these mechanisms under documented configurations and delimit what can be concluded from them.
...
The same mid tier met the measured structural criteria on all seven evaluated papers, with mean lexical claim support of 0.95 compared with 0.94 for the flagship tier, while reducing measured cost from \$0.921 to \$0.140.
These results support task-specific routing within the evaluated cases: the mid tier met the summary structure and lexical-support criteria, whereas the current evidence favors the flagship route for grounded question answering.
...
First initialization from each of three fixed arXiv seeds displayed five live
papers. Seed-to-briefing latency ranged from 28.46 to 41.73~s, with a median
of 36.50~s.
Ten three-event synchronization batches retained their pending state after
database reopening and an HTTP~503 response. The isolated backend accepted all 30 unique
events on first submission and identified all 30 replays as duplicates; the
replayed local rows also reached the synchronized state.
...
Two further arXiv seeds completed the seed-to-graph path, giving three fixed
seeds with a completed briefing and multi-node graph.

The final MVP acceptance session extended the same path through the ordinary
Android interface. The selected paper was saved, reopened, and shared; two
source-linked question turns remained in one paper conversation; a 50-paper
graph displayed 53 directed citations and responded to the References filter;
and one weekly notification opened the corresponding ten-paper briefing. All
32 tests in the expanded connected-device regression suite passed.
...
mixed. A graph displayed again in the same WebView had a lower session median
than its first display in all sixteen matched resource-by-node comparisons.
The graph measurements rose from one to twelve nodes and then followed no
monotonic pattern through 25 and 50 nodes. These results describe the controlled
...
Software tests verify node selection, \emph{Open Paper} navigation, and
selection restoration. The visible instruction and relation context are
implemented design responses, and participant retesting remains future work.
The study provides design-stage observations from five participants under
moderated prompts and fixed prototype content. Participant-level timing and
interaction records were not collected.

The evidence bears differently on the four contributions.
...
The Android client carries these properties through cached state, asynchronous polling, recoverable event delivery, and bounded graph interaction. The completed MVP extends the same flow with paper retention and sharing, paper-scoped follow-up questions, weekly re-entry, and graph filtering.
...
Several features of the evaluation bound the conclusions.
...
The resource measurements use one Android version and one emulator host, and the usability study contains five participants.
...
Under the recorded conditions, the mechanisms operated through the same mobile literature workflow.
...
polling, file-backed event retention and duplicate-safe upload, multi-node graph
interaction, and five-paper briefings for all three measured seeds. The final
acceptance path additionally covered Saved papers, public sharing, source-linked
follow-up questions, filtered citation exploration, and weekly-notification
re-entry.

The evidence supports a system contribution with bounded empirical results.
...
An AI literature assistant therefore needs inspectable boundaries around paper versions, retrieved evidence, inferred interests, and degraded mobile state.
Such boundaries make each reported output state available for systematic evaluation and improvement.
