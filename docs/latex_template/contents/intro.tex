...
The implemented behavior-v1 model is deliberately narrower than a multi-timescale memory.
It maintains one bounded, time-decayed normalized vector from declared implicit events; explicit topic editing remains separately inspectable and correctable.
Short-term and long-term state separation is reserved for planned behavior-v2 until a frozen implementation and E3 evidence are available.
...
TC3 & RQ3 & C3: explicit interests plus a bounded, time-decayed behavior-v1 vector with correction; multi-timescale behavior-v2 planned & E3: temporal recommendation, ablation, diversity, and correction tests & \pending \\
...
The third contribution is a bounded adaptation contract: behavior-v1 maps declared behavioral events to one bounded, time-decayed normalized vector, while users can inspect or correct the resulting state. Multi-timescale state remains a behavior-v2 plan rather than an implemented claim.
...
The available materials include a controlling thesis outline, acceptance criteria, an architecture design, a story map, an interactive HTML prototype, user-interface and user-experience (UI/UX) mockups, a final usability presentation, and an inspected source repository with exact implementation snapshots.
E1--E3 result artifacts and participant-level E4 records remain unavailable.
