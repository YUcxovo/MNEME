...
The integrated repository state is commit \texttt{70eb54c} of the MNEME repository, which contains the backend service, the AI service layer, the Android client, and their test suites.
...
Backend ingestion, jobs, and graph platform & \implemented{} (code) & Present with tests at \texttt{70eb54c}; behavioral claims pending Ruiyu's sign-off. E1/E3 research results \pending. \\
Android client, offline cache, event queue, graph UI & \implemented{} (code) & Present with tests at \texttt{70eb54c}; behavioral and device claims pending Heng's and Hanyang's sign-off. \\
...
Retrieval is revision-scoped: every chunk row binds to a \texttt{paper\_version\_id}, and an implemented API test asserts that Q\&A over one revision cannot return chunks of another.
These are software facts at \texttt{70eb54c}; they do not by themselves establish parser structure quality or invalidation completeness.
...
The evaluated configuration is the implemented pipeline at \texttt{70eb54c}, not an idealized design.
...
The implemented recommendation scorer at \texttt{70eb54c} blends three signals: explicit topic match (weight 0.45), cosine similarity between the user's learned behavior embedding and the paper's mean chunk embedding (weight 0.35), and publication recency with a one-week half-life (weight 0.2).
...
All dollar figures in this section are estimates from provider list prices (reviewed 2026-07) and token approximations, classified \observed{}; measured spend comes from the persisted cost telemetry once the deployed pipeline has run, and remains \pending.
...
Estimated costs derive from list prices and token approximations and may diverge from billed spend.
